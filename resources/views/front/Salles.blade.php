            ...
            <div class="bg-white p-6 rounded-lg shadow-lg">
                <div class="flex justify-between items-center mb-5">
                    ...
                    <button class="bg-gray-800 text-white px-4 py-2 rounded-lg hover:bg-gray-700"><a
                            href="/createSalle">Create</a></button>
                </div>
                <table class="min-w-full bg-white">
                    <thead>
                        ...
                        <tr>
                            ...
                            <th class="py-2 px-4 border-b">capacite</th>
                        ...
                        @foreach ($salles as $salle)
                            <tr>
                                <td class="py-2 px-5 border-b">
                                    <img src="{{ $salle->photo }}" class="w-10" alt="">
                                </td>
                                <td class="py-2 px-4 border-b">{{ $salle->name }}</td>
                                <td class="py-2 px-4 border-b">{{ $salle->numero }}</td>
                                <td class="py-2 px-4 border-b">{{ $salle->type }}</td>
                                <td class="py-2 px-4 border-b">{{ $salle->capacite }}</td>
                                <td class="py-2 px-4 border-b text-green-500">{{ $salle->status }}</td>
                                <td class="py-2 px-4 border-b">
                                    <a href="/Salle/update/{{ $salle->id }}"
                                        class="text-blue-500 hover:underline">Update</a>
                                </td>
                                <td class="py-2 px-4 border-b">
                                    <form action="" method="post">
                                        @csrf
                                        <input type="hidden" name="id" value="{{ $salle->id }}" id="">
